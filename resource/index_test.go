...
func TestIndexGetResource(t *testing.T) {
	...
	res, field, found := r.GetResource("foo", nil)
	assert.True(t, found)
	assert.Equal(t, "foo", res.name)
	assert.Equal(t, "", field)
	res, field, found = r.GetResource("foo.bar", nil)
	assert.True(t, found)
	assert.Equal(t, "bar", res.name)
	assert.Equal(t, "f", field)
	res, field, found = r.GetResource("foo.bar.baz", nil)
	assert.False(t, found)
	assert.Nil(t, res)
	assert.Equal(t, "", field)
	res, field, found = r.GetResource("bar", nil)
	assert.False(t, found)
	assert.Nil(t, res)
	assert.Equal(t, "", field)
	res, field, found = r.GetResource(".bar", foo)
	assert.True(t, found)
	assert.Equal(t, "bar", res.name)
	assert.Equal(t, "f", field)
	res, field, found = r.GetResource(".bar", nil)
