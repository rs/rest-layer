...
func TestProjectionEval(t *testing.T) {
	...
	p, err := pr.Eval(ctx, map[string]interface{}{"parent": map[string]interface{}{"child": "value"}, "simple": "value"}, s, nil)
	...
	p, err = pr.Eval(ctx, map[string]interface{}{"parent": map[string]interface{}{"child": "value"}}, s, nil)
	if assert.NoError(t, err) {
	...
	p, err = pr.Eval(ctx, map[string]interface{}{"with_params": "value"}, s, nil)
	if assert.NoError(t, err) {
	...
	p, err = pr.Eval(ctx, map[string]interface{}{"simple": "value"}, s, nil)
	assert.EqualError(t, err, "simple: field as no children")
	assert.Nil(t, p)
	// Deep field lookup on a field with invalid payload (no dict)
	pr = Projection{{Name: "parent", Children: Projection{{Name: "child"}}}}
	p, err = pr.Eval(ctx, map[string]interface{}{"parent": "value"}, s, nil)
