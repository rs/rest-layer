...
// Compiler is an interface defining a validator that can be compiled at run
// time in order to check validator configuration validity and/or prepare some
// data for a faster execution.
...
// Compile implements Compiler interface and call the same function on each
// field. Note: if you use schema as a standalone library, it is the *caller's*
// responsibility to invoke the Compile method before using Prepare or Validate
// on a Schema instance, otherwise FieldValidator instances may not be
// initialized correctly.
func (s Schema) Compile() error {
	// Search for all dependencies on fields, and compile then.
...
// existing data (from the current item) or data generated by the system thru "default" value
// or hooks.
//
...
// in the change map (instead of just being absent). This instruct the validator that the field
